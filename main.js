...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.shimmerTypes={
			...
			'golden':{
				...
				popFunc:function(me)
				{
					...
					if (Math.random()<0.6 && (Math.random()<0.05 || !Game.hasBuff('Dragonflight'))) list.push('click frenzy');
                    if (Math.random()<0.1) list.push('multiply cookies');
					if (me.wrath && Math.random()<0.1) list.push('cursed finger');
					
					if (Game.BuildingsOwned>=70 && Math.random()<0.25) list.push('building special');
					
					if (Game.canLumps() && Math.random()<0.01) list.push('free sugar lump');
					...
					{
						Game.gainLumps(2);
						popup=loc("Sweet!<br><small>Found 2 sugar lump!</small>");
					}
					...
					{
						buff=Game.gainBuff('click frenzy',Math.ceil(16*effectDurMod),777777);
					}
		...
		*/
		...
		Game.wrinklerHP=2.1;
		Game.wrinklerLimit=40;//hard limit regardless of boosts
		Game.wrinklers=[];
		...
		Game.SpawnWrinkler=function(me)
		{
			...
			me.type=0;
			me.clicks=0;
			if (Math.random()<0.01) me.type=1;//shiny wrinkler
			return me;
		}
		...
		Game.UpdateWrinklers=function()
		{
			...
			for (var i in Game.wrinklers)
			{
				...
				{
					var chance=0.001*Game.elderWrath;
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			UNLOCKING STUFF
			=======================================================================================*/
			if (Game.T%(Game.fps)==0 && Math.random()<1/10000) Game.Win('Just plain lucky');//1 chance in 1,000,000 every second achievement
