...
M.launch=function()
{
	...
	M.init=function(div)
	{
		...
		M.spells={
			'conjure baked goods':{
				...
				desc:loc("Summon half an hour worth of your CpS, capped at %1% of your cookies owned.",25),
			...
			'hand of fate':{
				...
				win:function()
				{
					...
					if (Game.BuildingsOwned>=70 && Math.random()<0.25) choices.push('building special');
					//if (Math.random()<0.2) choices.push('clot','cursed finger','ruin cookies');
					if (Math.random()<0.15) choices=['cookie storm drop'];
					if (Math.random()<0.01) choices.push('free sugar lump');
	...
	M.logic=function()
	{
		...
		M.magicPS=Math.max(0.002,Math.pow(M.magic/Math.max(M.magicM,100),0.5))*0.02;
		M.magic+=M.magicPS;
		M.magic=Math.min(M.magic,M.magicM);
		if (Game.T%5==0)
		{
